  ...
  animations: [
    trigger('inOut', [
      ...
      transition('out => in', [
        ...
        animate(
          '250ms ease-in-out',
          keyframes([style({ offset: 0, opacity: 0 }), style({ offset: 1, opacity: 1 })])
        ),
      ]),
      ...
      transition('in => out', [
        animate(
          '250ms ease-in-out',
          keyframes([style({ offset: 0, opacity: 1 }), style({ offset: 1, opacity: 0 })])
        ),
      ]),
    ]),
  ],
...
export class SpinnerComponent implements OnInit, OnDestroy {
  ...
  $state = 'out';
  MtxLoaderType = 'spinner';
  typeLoader: MtxLoaderType = 'spinner';
